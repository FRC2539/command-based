 ...
 */
SanePIDController::SanePIDController(float Kp, float Ki, float Kd,
 ...
 * integral and differental terms. The default is 50ms.
 */
SanePIDController::SanePIDController(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period) :
	m_semaphore (0)
{
...
void SanePIDController::Initialize(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period)
{
	m_table = NULL;
	...
	m_controlLoop = new Notifier(SanePIDController::CallCalculate, this);
 ...
 * Free the PID object
 */
SanePIDController::~SanePIDController()
{
 ...
 */
void SanePIDController::CallCalculate(void *controller)
{
	SanePIDController *control = (SanePIDController*) controller;
	control->Calculate();
}
  ...
  */
void SanePIDController::Calculate()
{
	bool enabled;
	PIDSource *pidInput;
	PIDOutput *pidOutput;
 ...
 */
void SanePIDController::SetPID(float p, float i, float d)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void SanePIDController::SetPID(float p, float i, float d, float f)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void SanePIDController::SetContinuous(bool continuous)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void SanePIDController::SetInputRange(float minimumInput, float maximumInput)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void SanePIDController::SetOutputRange(float minimumOutput, float maximumOutput)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Set the setpoint for the SanePIDController
 * @param setpoint the desired setpoint
 */
void SanePIDController::SetSetpoint(float setpoint)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Returns the current setpoint of the SanePIDController
 * @return the current setpoint
 */
float SanePIDController::GetSetpoint()
{
	float setpoint;
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
float SanePIDController::GetError()
{
	float error;
	double pidInput;
	CRITICAL_REGION(m_semaphore)
 ...
 */
void SanePIDController::SetTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 * @param percentage error which is tolerable
 */
void SanePIDController::SetPercentTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_toleranceType = kPercentTolerance;
 ...
 * @param percentage error which is tolerable
 */
void SanePIDController::SetAbsoluteTolerance(float absTolerance)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
bool SanePIDController::OnTarget()
{
	bool temp;
...
/**
 * Begin running the SanePIDController
 */
void SanePIDController::Enable()
{
	CRITICAL_REGION(m_semaphore)
	{
		m_enabled = true;
...
/**
 * Stop running the SanePIDController, this sets the output to zero before stopping.
 */
void SanePIDController::Disable()
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Return true if SanePIDController is enabled.
 */
bool SanePIDController::IsEnabled()
{
	bool enabled;
	CRITICAL_REGION(m_semaphore)
	{
...
std::string SanePIDController::GetSmartDashboardType(){
	return "SanePIDController";
}

void SanePIDController::InitTable(ITable* table){
	if(m_table!=NULL)
...
ITable* SanePIDController::GetTable(){
	return m_table;
}

void SanePIDController::ValueChanged(ITable* source, const std::string& key, EntryValue value, bool isNew){
...
void SanePIDController::UpdateTable() {

}

void SanePIDController::StartLiveWindowMode() {
	Disable();
}

void SanePIDController::StopLiveWindowMode() {
